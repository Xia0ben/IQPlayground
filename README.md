...
- Learn Python (at the beginning, we were four out of five never to have programmed a full project in this language), and how to write decent python as compliant as possible with PEP8,
- Implement many of the Indexation &amp; Query Algorithms that are used nowadays in most software,
- Think about how the activation and parameterizing of these algorithms influenced the overall performance,
- And naturally spend time trying to understand each other and work together as a team.
...
- Clone the project:
```bash
git pull https://github.com/Xia0ben/IQPlayground.git
```
- If you have access to the "latimes" archive with all the files, you may want to uncompress it in the project's root (careful to remove the MACOSX subfolder and put all the content in the main latimes folder, and remove the .txt files manually : the files shouldn't be in "latimes/latimes" but in "latimes").
- Install PyGObject, GTK+ and their dependencies following the instructions at the following link : http://pygobject.readthedocs.io/en/latest/getting_started.html
- Install nltk and gi (using the python interpreter provided by the previous link):
```python
>>> import nltk
>>> import gi
>>> nltk.download()
```
- Install the numpy and sortedcontainers librairies with pip :
```bash
pip install numpy --index-url=http://pypi.python.org/simple/ --trusted-host pypi.python.org
pip install sortedcontainers
```

> If at execution time, the project fails and says that you are missing the "pickles" and "invertedfiles" folders in the project's root directory, create them manually.
...
+ Stats
  ...
  - Contains classes which allow to get stats on indexing and querying
